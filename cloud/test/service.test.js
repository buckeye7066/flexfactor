...
test("the reusable workflow is pinned to the release that carries this client", () => {
  assert.equal(ENGINE_REF, "android-v3.5.16");
  assert.match(
    mobileWorkflow(),
    /^[ \t]*uses: buckeye7066\/flexfactor\/\.github\/workflows\/mobile-run\.yml@android-v3\.5\.16$/m,
  );
...
for (const newerRef of ["android-v3.5.17", "android-v3.6.0", "android-v4.0.0"]) {
...
test("downgrade protection has a distinct cloud release identity", () => {
  assert.equal(SERVICE_VERSION, "1.1.17");
});
