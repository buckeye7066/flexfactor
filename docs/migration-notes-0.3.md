...
- Completion is stricter. A previously green static review may now exit non-zero when tests collected zero cases, function/workflow evidence is incomplete, changed-file rescan or blast-radius analysis fails, or the automated evidence does not bind to the exact commit.
