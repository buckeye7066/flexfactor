...
## Verified worker packaging and model attribution

The owner worker is now included in Python distributions under `providers/owner_ai`. Both Windows and Linux resolve their native executable and an explicit private Codex home. A managed Codex session cannot recursively launch the owner worker. Planning sees the worker's 32,000-token accepted-response ceiling and 190,000-byte request limit before choosing whole-file repair. Oversized files retain the anchored-edit path instead of assuming a 128,000-token whole-file response.

Codex inference uses its official stdio app-server. The transport verifies ChatGPT account type, the thread's actual selected model/provider, read-only sandbox, no instruction files, and no model-reroute notification. System instructions retain developer priority; user source remains task input. Non-tool reasoning lifecycle events are accepted without becoming output. Legacy JSONL that cannot establish the executed model is not labelled as verified model evidence.

A controlled live proof completed in 4.14 seconds: `FlexFactor verified model OK.`, subscription billing, actual configured `gpt-6-astra`, and usage from the official turn. The built wheel was inspected and contained all three runtime modules. This does not imply a hard provider-side output-token quota: Codex does not expose that API control; local request/response byte limits, response-size validation and cancellation remain enforced. No API key fallback was used.
