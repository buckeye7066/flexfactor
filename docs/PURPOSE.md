...
| Send source to a cloud provider | Egress gate `flexfactor_egress` on every repo-derived payload; `--redact`, `--allow-sensitive`, `FLEXFACTOR_ALLOW_EGRESS`, policy.json `allow_egress`; `--model-mode local` removes paid credentials |
