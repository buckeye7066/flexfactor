...
Assigned Applications: Claude Code
Claude Code owns these nine applications and no others:
Family Stewardship Navigator
Repo buckeye7066/family-stewardship-navigator, default main; launcher Start-Family-Stewardship.cmd.
Goal: a dependable family finance, estate, fiduciary, document, tax-year, and stewardship workspace with strong encryption, permissions, backups, provenance, recovery, and understandable workflows.
Acceptance: secure first run; correct household/role boundaries; encrypted backups and tested restore; no plaintext secrets; correct tax-year/version handling; concurrency and stale-write protection; export/delete; disaster recovery; clean launcher.
Are We Mice Or Are We Men
Repo buckeye7066/are-we-mice-or-are-we-men, default main; Vercel frontend and Railway backend must be reverified.
Actual purpose: an invite-only, supervised AEMT and paramedic education platform, not mouse/human research software and not real-time clinical decision support.
Acceptance: correct product identity everywhere; authoritative dated curriculum provenance; qualified educator/clinician review status; AEMT and paramedic blueprint coverage; realistic cases, vitals, labs, quizzes, and exams; no unsafe field-care framing; secure accounts; privacy/export/delete; representative supervised learner journeys on exact deployed SHA.
Incognito
Repo buckeye7066/incognito, default main; launcher launch.bat; optional private sync must be reverified.
Goal: a one-household, local-first encrypted identity/privacy vault that remains useful offline, makes sync optional and zero-knowledge, minimizes metadata, and supports safe recovery and transactional rekey.
Acceptance: truthful local/backup/sync modes; memory-only keys where intended; encrypted backups; interrupted-rekey recovery; old-key invalidation; multi-device conflict handling; metadata minimization; offline use; real multi-device proof if sync is released.
ForgePress
Repo buckeye7066/ForgePress, default master; installed EXE is behavioral reference only until reproducible source build is proven.
Goal: a safe, reproducible publishing workstation for manuscript creation, export, ISBN/Bowker, repository publishing, detector integrations, versioning, and release receipts.
Acceptance: clean source build/package; previews and print/PDF work; credentials in OS storage; allowlisted IPC/network; publication receipts bound to action, target, freshness, and one-time use; Bowker cannot bypass confirmation; saves await persistence; supported detectors work; signed installer remains a blocker until completed.
Family Castle Clash
Repo buckeye7066/family-castle-clash, default main; local launcher must be discovered.
Goal: a private family-themed real-time lane battler with family-admin enrollment, server-authoritative and restart-safe play, privacy-preserving media import, understandable cards/powers, and no unintended public exposure.
Acceptance: invite-only registration; role/session revocation; private-LAN default; TLS if exposed; actual EXIF removal; transactional state; simultaneous-write, crash, restart, reconnect, and match-resume proof; deterministic simulation; hidden-hand and anti-cheat guarantees; polished gameplay and character explanations.
IPlay
Repo buckeye7066/Iplay, default main; current local launcher/source must be reverified.
Goal: produce a lifelike, full-length video of the chosen avatar genuinely performing the selected music, with correct instrument-specific fingering, keys, bowing, strokes, strums, picks, timing, continuity, scene direction, and original master audio, with no source performer leakage.
Acceptance: piano, guitar, and violin reference journeys; actual generated video inspected; master sample-rate timeline correctness; right/left-hand semantics; checkpoint identity binding; deterministic resume; incremental checkpoints; cancellation; mandatory provenance sidecars before publish; audio hash authority; no generic beat motion or source-body fallback; clean Windows import/analyze/preview/render/resume/export; GPU or provider requirement remains a blocker until real render proof passes.
Kidney Antigen Discovery
Local source C:\Users\firer\kidney-antigen-discovery; private repo only after source, secret, license, and data review.
Goal: a research-only reproducible antigen-prioritization workbench using versioned human tissue/cell evidence, surface accessibility, disease/graft context, off-target expression, immunobiology, and explicit uncertainty, never clinical validation.
Acceptance: deterministic startup; full dataset/model/equation inventory; source/species/isoform/release provenance; missingness not converted to zero; reproducible fixtures; threshold sensitivity; alternative explanations; benchmark candidates; no clinical efficacy or transplant-treatment claim; scientific review record.
CRISPR Compass
Local source C:\Users\firer\crispr-compass; observed local target http://localhost:8501; private repo only after source/scientific review.
Goal: a clearly bounded research/education tool whose actual sequence/evidence functions are verified from source, reproducible, secure, and non-clinical.
Acceptance: deterministic startup; visible research-only boundary; actual modules and workflows inventoried; genome build, nuclease/PAM, transcript/isoform, scoring version, data sources, assumptions, uncertainty, and off-target limits recorded where relevant; sequence/privacy controls; reproducible fixtures; no inferred capabilities from the product name.
App Store Publisher
Repo buckeye7066/app-store-publisher, default main; launcher launch.vbs.
Goal: a safe release orchestrator for Google Play, Samsung, and Apple workflows that validates artifacts and metadata, prefers supported APIs, uses browser automation only as a bounded fallback, requires fresh approval for each irreversible action, and preserves receipts and reconciliation.
Acceptance: complete release-state model; artifact/signature/version/hash validation; dry-run zero mutation; isolated browser profile; allowlisted domains/actions; redacted logs; resumable checkpoints; 2FA handoff; duplicate/replay prevention; internal/test-track submission with exact store receipt; no promotion or rollout without fresh specific approval.
Claude-Specific Rule
Use one Claude Code session and one active application.
Do not invoke nested claude commands, background Claude sessions, parallel task agents, or another IDE. Do not modify %USERPROFILE%\.fcc, global Ollama service configuration, Cursor installation, or AI Time because those belong to Cursor.
For scientific applications, separate verified evidence, biological plausibility, hypothesis, model output, uncertainty, and clinical relevance. Never convert research ranking into clinical advice.
Axiom Portfolio | Claude Code | One Program at a Time
CLAUDE CODE SINGLE-PROGRAM PRODUCTION EXECUTION DIRECTIVE
Axiom application portfolio: Claude Code lane
Prepared 9 August 2026
NON-NEGOTIABLE EXECUTION LOCK: EXACTLY ONE ACTIVE_APP
The current program must be completed or truthfully blocked, fully checkpointed, and released from ownership before the next assigned program is touched.
You are the top-level Claude Code executor responsible for the applications assigned to this portfolio lane. Your task is not merely to audit, recommend, plan, create pull requests, or produce reports. Your task is to take each assigned program, one at a time, to the strongest truthful release state supported by implementation and evidence.
...
For every program assigned to Claude Code:
...
Derive and reconcile the assigned inventory from this directive, repository records, current workspace folders, deployment records, desktop launchers, existing issues, pull requests, branches, audits, and readiness reports. Create or maintain `PORTFOLIO_CLAUDE_PRODUCTION_READINESS.md`.
...
1
Family Stewardship Navigator
Assigned to this executor only; sequential ACTIVE_APP
2
Are We Mice Or Are We Men
Assigned to this executor only; sequential ACTIVE_APP
3
...
ACTIVE_APP
Exactly one assigned program, or NONE while transitioning
ACTIVE_REPOSITORY
The verified source-of-truth repository for ACTIVE_APP
ACTIVE_BRANCH
Use `claude/production-ready/<program-slug>` unless the repository workflow requires another reviewed branch
ACTIVE_WORKTREE
One isolated worktree or checkout for ACTIVE_APP; never shared with another executor
NEXT_APP
The next queued program, not yet touched
...
Use one Claude Code session and one active application. Do not invoke nested claude commands, teams, subagents, background Claude sessions, or another IDE.
Do not run multiple application implementations concurrently. Long tests or renders belong to the current ACTIVE_APP and do not justify starting another program.
Maintain a durable checkpoint before context compaction, restart, or handoff. The checkpoint must include repository, branch, worktree, current SHA, changed files, tests, findings, and next action.
Do not modify Cursor-owned local AI infrastructure, FCC/Ollama global configuration, AI Time, or ChatGPT-owned cloud applications unless the user explicitly reassigns ownership.
For scientific or research applications, separate verified evidence, biological plausibility, hypothesis, model output, uncertainty, and clinical relevance. Never convert a research ranking into clinical advice.
AVAILABLE TOOLING AND EVIDENCE DISCIPLINE
Use the local shell, repository tools, Git, GitHub CLI, test runners, packaging tools, and supported deployment interfaces available to Claude Code.
Inspect source before inferring purpose from a product name. Preserve local user data and uncommitted work.
When the current application needs a fresh review, perform it sequentially after implementation from a clean review checklist or an external repository review service. Do not create a concurrent Claude worker.
...
Maintain `PORTFOLIO_CLAUDE_PRODUCTION_READINESS.md` for the assigned lane and `docs/production-readiness/<program-slug>.md` for the current program where the repository structure permits.
...
Do not declare the Claude Code lane complete until every assigned program has:
A Purpose Contract
...
2. Create or reconcile `PORTFOLIO_CLAUDE_PRODUCTION_READINESS.md` from the assigned queue.
...
APPENDIX A: ASSIGNED APPLICATIONS FOR CLAUDE CODE
...
1. Family Stewardship Navigator
Repo buckeye7066/family-stewardship-navigator, default main; launcher Start-Family-Stewardship.cmd.
Goal: a dependable family finance, estate, fiduciary, document, tax-year, and stewardship workspace with strong encryption, permissions, backups, provenance, recovery, and understandable workflows.
Acceptance: secure first run; correct household/role boundaries; encrypted backups and tested restore; no plaintext secrets; correct tax-year/version handling; concurrency and stale-write protection; export/delete; disaster recovery; clean launcher.
2. Are We Mice Or Are We Men
Repo buckeye7066/are-we-mice-or-are-we-men, default main; Vercel frontend and Railway backend must be reverified.
Actual purpose: an invite-only, supervised AEMT and paramedic education platform, not mouse/human research software and not real-time clinical decision support.
Acceptance: correct product identity everywhere; authoritative dated curriculum provenance; qualified educator/clinician review status; AEMT and paramedic blueprint coverage; realistic cases, vitals, labs, quizzes, and exams; no unsafe field-care framing; secure accounts; privacy/export/delete; representative supervised learner journeys on exact deployed SHA.
3. Incognito
