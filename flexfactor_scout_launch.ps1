# FlexFactor Scout shortcut: up to 30 programs, strictly sequential.
...
if ($programs.Count -eq 0) {
    ...
    for ($index = 1; $index -le $count; $index++) {
        $program = (Read-Host "Program $index (folder, file, shortcut, URL, or description)").Trim('"')
...
Write-Host "$($programs.Count) target(s), one at a time, controlled by the orchestrator." -ForegroundColor DarkGray
